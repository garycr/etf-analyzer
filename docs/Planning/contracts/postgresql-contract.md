...
| 6 | `609dcf7bd1faa472e850b7ed1cd32511987a596e7f52403ede1b832fc1e4e8dc` | `af70acc24cdf99a0f4ca60b4c6c51eceaf8a26609369228db063d30d62e35d42` |
...
**2026-09-18 CT-LED-015 amendment.** Sequence 6 exposes only the permitted parent entry points: `app_runtime` may execute `ledger_append`, `projection_runtime` may execute `projection_publish`, and the existing sequence-3 grant leaves `audit_append` solely available to `audit_runtime` among runtime roles. Direct anchor execution, caller/outcome mismatches, table DML, and owner-role inheritance remain denied before persistence. The prototype remains inactive and empty-database-only, so sequence 6 was re-baselined in place without changing earlier migration identities.
...
| `0005-analytics-evidence` | `analytics_input_sets`, `analytics_evidence_bundles`, `analytics_manifests`, `analytics_lifecycle_references`, `analytics_deletion_links`, `analytics_retention_bindings`, `analytics_evidence_replays`, `analytics_publications`, `analytics_audit`; private `_evidence_rfc8785`, controlled `evidence_commit`, `evidence_read`; evidence indexes below |
...
The five views are owned by `schema_owner`, use `security_barrier=true`, and have these complete ordered columns and query definitions: `current_watchlist(instrument_id,display_name,validation_state,position,version)` selects those columns from `etf.watchlist_items`; `current_jobs(job_id,job_type,status,restartability,attempt,created_at,started_at,completed_at,accepted_count,rejected_count)` selects those columns from `etf.jobs`; `current_paper_orders(order_id,instrument_id,state,aggregate_version,research_evidence_id,side,requested_quantity,filled_quantity,open_quantity,unit_price,trade_date)` selects those columns from `etf.paper_orders`; `current_portfolios(portfolio_id,portfolio_version,baseline_version,precision_policy_version)` selects those columns from `etf.portfolios`; `current_analytics_publications(publication_target_id,publication_version,evidence_id,published_at,bundle_hash)` selects those columns from `etf.analytics_publications`. Every query is a plain projection with no predicate, join, ordering, function, or caller-dependent expression.

Cross-owner authority is part of the exact object-grant closure. `ledger_writer_owner` receives column-level SELECT on `paper_orders(order_id,instrument_id,side,aggregate_version)` solely to validate a Fill payload against the authoritative locked order. `schema_owner` receives column-level SELECT on exactly the projected columns of the five reader-view base tables so PostgreSQL can execute its owner-checked views. For reconciliation and `portfolio_get`, `projection_owner` receives column-level SELECT on `portfolios(portfolio_id,portfolio_version,baseline_version,precision_policy_version)`, `ledger_effects(portfolio_id,effect_type,instrument_id,lot_id,quantity,money)`, `ledger_lots(portfolio_id,lot_id,instrument_id)`, `ledger_allocations(portfolio_id,sell_transaction_id,effect_ordinal,lot_id,consumed_quantity,allocated_basis)`, `ledger_commitments(portfolio_id,ledger_sequence,commitment_hash)`, and `ledger_anchors(portfolio_id,ledger_sequence,commitment_hash)` so projection publication can rebuild exact ledger state and only an anchored source commitment is eligible. `anchor_owner` receives column-level SELECT on `audit_commitments(audit_sequence,audit_commitment)` plus table-level INSERT solely for atomic commitment construction; immutable guards deny update, delete, and truncate. Nested EXECUTE grants are `ledger_append(jsonb)` to `application_writer_owner`; `audit_append(jsonb)` to `application_writer_owner`, `ledger_writer_owner`, and `projection_owner`; and `anchor_append(jsonb)` to `ledger_writer_owner`, `audit_writer_owner`, and `projection_owner`. The projection owner may invoke only the read-only `Verify` variant of `anchor_append`; caller/variant enforcement precedes mutation. No other cross-owner base-table authority exists.
...
All unspecified columns have no default. Every FK is `MATCH SIMPLE ON UPDATE RESTRICT ON DELETE RESTRICT NOT DEFERRABLE`; any null child column exempts that row from the FK comparison. Names are `pk_<table>`, `fk_<child>__<child-columns>__<parent>`, `uq_<table>__<columns>`, `ck_<table>__<rule>`, and `ix_<table>__<columns>`. When an expanded fixture identifier would exceed PostgreSQL's 63-byte limit, its exact catalog name uses these tokens: `dataset_id=ds`, `dataset_version=ver`, `ingestion_job_id=job`, `instrument_id=inst`, `trading_date|observation_date=date`, `provider_id=prov`, `adjustment_policy=adj`, `source_available_at=available`, `release_timestamp=release|rel`, `revision=rev`, `vintage_id=vtg`, and `raw_source_hash=hash`; the shorter `rel` and `vtg` forms are used only when the other tokens still exceed the limit. Analytics identifiers use `analytics_evidence_bundles=bundles`, `analytics_input_sets=input_sets`, `evidence_id=evidence`, `publication_target_id=publication`, and omit repeated `analytics_` qualifiers only when the expanded name would exceed 63 bytes. Indexes are btree with keys in listed order and no predicate unless listed. Cross-row ordinals/chains are enforced by controlled procedures under owner locks and revalidated by CT-DB-001.
...
| `analytics_manifests` | `manifest_id text! PK`, `manifest_sequence bigint! UQ`, `previous_manifest_hash char(64)?`, `evidence_id text!`, `retention_policy_version text!`, `retention_epoch timestamptz!`, `canonical_content jsonb!`, `manifest_hash char(64)!`; FK/JSON registries, positive contiguous sequence, first predecessor null and later predecessor required, lowercase hashes, immutable |
...
| `analytics_retention_bindings` | `evidence_id text!`, `retention_epoch timestamptz!`, `target_class text!`, `retain_through timestamptz!`, `lifecycle_state text!`, `retention_policy_version text!`; `PK(evidence_id,retention_epoch,target_class)`, immutable history |
