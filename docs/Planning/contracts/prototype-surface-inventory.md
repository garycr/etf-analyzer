...
**Inventory version:** `1.0.0-candidate.4`
...
| Process-crossing API | Yes | Yes | `docs/Planning/contracts/openapi-contract.yaml` `1.0.0-candidate.3`; executable `CT-API-001A..L` | Versioned localhost browser-to-API request/response boundary only; no public ingress, callbacks, webhooks, providers, brokers, or events | WP-4 closed by DEC-039; REV-098/099 PASS and REV-100 conditional PASS |
...
| `PT-CONTRACT-SCOPE-API` | PASS (qualified: WP-8 hardening) | OpenAPI candidate.3 and executable `CT-API-001A..L` pass Architecture and Code review; Security review has no Critical/Major findings. | REV-100 timeout/auditability Minors remain WP-8 hardening obligations; any API drift or non-loopback ingress reopens the guard. |
