...
The schema manifest is the canonical UTF-8 RFC 8785 JSON projection of all `etf` schemas, tables, ordered columns, types, nullability, defaults, primary/foreign/unique/check constraints, indexes, functions, triggers, roles, ownership, and grants after each migration. It excludes PostgreSQL-generated OIDs and physical storage parameters. The first stored manifest is projected only from the committed post-`0001` state; the empty provisioner prerequisite has no migration row or manifest hash. Ring 2 migration artifacts must publish the six SQL content hashes and six resulting manifest hashes before execution; until then `CT-DB-001` is a design-time plan, not passing migration evidence.
...
| `0002-application` | `application_replays`, `watchlist_items`, `jobs`, `job_checkpoints`, `readiness_audit`; `application_replay_get_or_put`, `watchlist_write`, `job_start`, `job_restart`, `readiness_append` |
