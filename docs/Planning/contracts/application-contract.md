...
`statusText` is the visible heading and programmatic accessible name. `causeText` is a visible, programmatically associated description. A renderer uses `aria-labelledby` and `aria-describedby` or equivalent native relationships rather than concatenating both into an opaque label.
...
The fresh `transitionCommandId` is generated only after explicit activation and remains fixed across transport retries of that activation. `expectedVersion` is copied from the presented `PaperOrder.aggregateVersion`; a changed authoritative version therefore produces unchanged `ORDER_VERSION_CONFLICT`. Missing required identity/context, denied access, or absence of a safe catalog operation yields `recovery: null`, no target payload, and a plain-language escalation message that explains a safe next step without inventing an operation. These states retain focus on the trigger while the status or alert is announced. The application never invents an operation, dispatches a null recovery, or embeds an unbounded value.

Paper mutation confirmation is presented as a keyboard-contained modal or equivalent blocking confirmation surface. Initial focus targets its heading or non-destructive cancel control, never the submit control; Escape cancels; focus returns to the trigger on cancel; and explicit confirmation alone activates the transition.
