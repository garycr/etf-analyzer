...
- Key generation occurs outside application workloads through the local deployment secret workflow. A deployment-only job injects usable HMAC key bytes into an anchor-owner-only PostgreSQL store and removes the mounted Kubernetes Secret after successful injection. The legacy `key_ciphertext` column name does not imply application-level wrapping; database and backup storage provide encryption at rest. The key is never readable by API, portfolio, ledger-writer, projection-writer, audit, diagnostics, or backup logs.
