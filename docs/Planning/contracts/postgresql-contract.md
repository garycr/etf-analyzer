...
Cross-owner authority is part of the exact object-grant closure: `ledger_writer_owner` receives column-level SELECT on `paper_orders(order_id,instrument_id,side,aggregate_version)` solely to validate a Fill payload against the authoritative locked order. Nested EXECUTE grants are `ledger_append(jsonb)` to `application_writer_owner`; `audit_append(jsonb)` to `application_writer_owner`, `ledger_writer_owner`, and `projection_owner`; and `anchor_append(jsonb)` to `ledger_writer_owner`, `audit_writer_owner`, and `projection_owner`. The projection owner may invoke only the read-only `Verify` variant of `anchor_append`; caller/variant enforcement precedes mutation. No cross-owner base-table SELECT or other owner grant exists.
