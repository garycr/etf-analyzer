Feature: PT-APP-001 transport-independent application boundary conformance
  ...
  Scenario Outline: PT-APP-001M every post-submission transition has one closed payload schema
    Given PaperOrderTransition selects transition "<transition>"
    When exact transition payload '<validPayload>' is dispatched
    Then the owning paper-order contract is invoked exactly once
    When exact transition payload '<malformedPayload>' is dispatched
    Then the command or query fails with "APPLICATION_REQUEST_INVALID"
    And no owning contract or side effect is invoked

    Examples:
      | transition | validPayload                                                                                                                                                                                                                                                                                                                                 | malformedPayload                                                                                                                                                                                                                                                                                                          |
