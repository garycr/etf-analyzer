...
If the schema is missing, has the wrong owner, contains an object, exposes an ACL, or has a schema-scoped default privilege, stop migration processing with `APPLICATION_MIGRATIONS_INCOMPLETE`. An authorized operator must inspect the catalog evidence, determine origin and impact, and explicitly restore or recreate the prerequisite under the environment change process. Destructive cleanup is never automatic.
