...
BEGIN
  ...
  IF domain_name = 'Order' THEN
    ...
    IF session_user = 'audit_runtime' THEN
      IF outcome_name = 'IntentRecorded' THEN
        IF subject ->> 'oldOrderVersion' IS NULL OR subject ->> 'oldOrderVersion' !~ '^(0|[1-9][0-9]*)$' OR EXISTS (SELECT 1 FROM etf.order_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid) THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
      ELSE
        IF NOT EXISTS (SELECT 1 FROM etf.order_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND order_id = (subject ->> 'orderId')::uuid AND action = payload ->> 'action' AND correlation_id = (payload ->> 'correlationId')::uuid AND old_order_version = (subject ->> 'oldOrderVersion')::bigint AND outcome = 'IntentRecorded') OR EXISTS (SELECT 1 FROM etf.order_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND outcome <> 'IntentRecorded') THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
      END IF;
    END IF;
  ...
  ELSIF domain_name = 'Ledger' THEN
    ...
    IF session_user = 'audit_runtime' THEN
      IF outcome_name = 'IntentRecorded' THEN
        IF subject ->> 'oldPortfolioVersion' IS NULL OR subject ->> 'oldPortfolioVersion' !~ '^(0|[1-9][0-9]*)$' OR (subject ->> 'oldOrderVersion' IS NOT NULL AND subject ->> 'oldOrderVersion' !~ '^(0|[1-9][0-9]*)$') THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
        IF EXISTS (SELECT 1 FROM etf.ledger_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid) THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
      ELSE
        IF NOT EXISTS (SELECT 1 FROM etf.ledger_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND portfolio_id = (subject ->> 'portfolioId')::uuid AND action = payload ->> 'action' AND correlation_id = (payload ->> 'correlationId')::uuid AND old_portfolio_version IS NOT DISTINCT FROM (subject ->> 'oldPortfolioVersion')::bigint AND old_order_version IS NOT DISTINCT FROM (subject ->> 'oldOrderVersion')::bigint AND outcome = 'IntentRecorded') THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
        IF outcome_name IN ('Rejected','IntegrityFailed') AND EXISTS (SELECT 1 FROM etf.ledger_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND outcome <> 'IntentRecorded') THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
        IF outcome_name = 'TimeoutRecovery' AND EXISTS (SELECT 1 FROM etf.ledger_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND outcome <> 'IntentRecorded') THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
        IF outcome_name = 'RecoveryCompleted' AND (NOT EXISTS (SELECT 1 FROM etf.ledger_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND outcome = 'TimeoutRecovery') OR EXISTS (SELECT 1 FROM etf.ledger_audit WHERE attempt_intent_id = (payload ->> 'attemptIntentId')::uuid AND outcome = 'RecoveryCompleted')) THEN RAISE EXCEPTION 'AUDIT_REQUEST_INVALID' USING ERRCODE = '22023'; END IF;
      END IF;
    END IF;
...
BEGIN
  ...
  IF kind IN ('CashDeposit','CashWithdrawal') THEN
    IF jsonb_typeof(payload -> 'amount') = 'string' AND payload ->> 'amount' ~ '^(0|[1-9][0-9]*)\.[0-9]{9,}$' THEN RAISE EXCEPTION 'LEDGER_EXCESS_SCALE' USING ERRCODE='22023'; END IF;
    IF jsonb_typeof(payload -> 'amount') IS DISTINCT FROM 'string' OR payload ->> 'amount' !~ '^(0|[1-9][0-9]*)\.[0-9]{8}$' OR (payload ->> 'amount')::numeric <= 0 THEN RAISE EXCEPTION 'LEDGER_INVALID_DECIMAL' USING ERRCODE='22023'; END IF;
    IF (payload ->> 'amount')::numeric > 9000000000000000.00000000 THEN RAISE EXCEPTION 'LEDGER_BOUND_EXCEEDED' USING ERRCODE='22023'; END IF;
  ELSIF kind IN ('BuyFill','SellFill') THEN
    IF (jsonb_typeof(payload -> 'quantity') = 'string' AND payload ->> 'quantity' ~ '^(0|[1-9][0-9]*)\.[0-9]{11,}$') OR (jsonb_typeof(payload -> 'unitPrice') = 'string' AND payload ->> 'unitPrice' ~ '^(0|[1-9][0-9]*)\.[0-9]{11,}$') OR (jsonb_typeof(payload -> 'fee') = 'string' AND payload ->> 'fee' ~ '^(0|[1-9][0-9]*)\.[0-9]{9,}$') THEN RAISE EXCEPTION 'LEDGER_EXCESS_SCALE' USING ERRCODE='22023'; END IF;
    IF jsonb_typeof(payload -> 'quantity') IS DISTINCT FROM 'string' OR payload ->> 'quantity' !~ '^(0|[1-9][0-9]*)\.[0-9]{10}$' OR jsonb_typeof(payload -> 'unitPrice') IS DISTINCT FROM 'string' OR payload ->> 'unitPrice' !~ '^(0|[1-9][0-9]*)\.[0-9]{10}$' OR jsonb_typeof(payload -> 'fee') IS DISTINCT FROM 'string' OR payload ->> 'fee' !~ '^(0|[1-9][0-9]*)\.[0-9]{8}$' THEN RAISE EXCEPTION 'LEDGER_INVALID_DECIMAL' USING ERRCODE='22023'; END IF;
    IF (payload ->> 'quantity')::numeric <= 0 OR (payload ->> 'unitPrice')::numeric <= 0 OR (payload ->> 'fee')::numeric < 0 THEN RAISE EXCEPTION 'LEDGER_INVALID_DECIMAL' USING ERRCODE='22023'; END IF;
    IF (payload ->> 'quantity')::numeric > 1000000000.0000000000 OR (payload ->> 'unitPrice')::numeric > 1000000.0000000000 OR (payload ->> 'fee')::numeric > 1000000000.00000000 THEN RAISE EXCEPTION 'LEDGER_BOUND_EXCEEDED' USING ERRCODE='22023'; END IF;
  END IF;
  IF kind IN ('CashDeposit','CashWithdrawal') THEN
  ...
  PERFORM pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('etf:portfolio:' || portfolio::text,0));
  ...
  IF kind IN ('CashDeposit','CashWithdrawal') THEN
    IF payload - ARRAY['canonicalContent','correlationId','effectiveAt','expectedPortfolioVersion','keyIdentifier','portfolioId','transactionId','type','amount']::text[] <> '{}'::jsonb THEN RAISE EXCEPTION 'LEDGER_REQUEST_INVALID' USING ERRCODE='22023'; END IF;
  ...
  ELSIF kind IN ('BuyFill','SellFill') THEN
    ...
    cash_delta:=gross_value*CASE WHEN kind='BuyFill' THEN -1 ELSE 1 END-fee_value;
    IF COALESCE((SELECT sum(money) FROM etf.ledger_effects WHERE portfolio_id=portfolio AND effect_type='Cash'),0)+cash_delta < 0 THEN RAISE EXCEPTION 'LEDGER_INSUFFICIENT_CASH' USING ERRCODE='P0001'; END IF;
  ...
  IF kind='BuyFill' THEN
    intended_effects := intended_effects || jsonb_build_array(
      ...
      jsonb_build_object('effect_ordinal',2,'effect_type','Basis','instrument_id',payload ->> 'instrumentId','lot_id',(payload ->> 'fillId')::uuid,'source_effect_id',NULL,'quantity',NULL,'money',gross_value+fee_value,'rate',NULL));
  ELSIF kind='SellFill' THEN
    ...
    FOR lot_record IN SELECT lot.*,COALESCE((SELECT sum(effect.quantity) FROM etf.ledger_effects effect WHERE effect.portfolio_id=lot.portfolio_id AND effect.lot_id=lot.lot_id AND pg_catalog.right(effect.effect_type,8)='Position'),0) AS available,COALESCE((SELECT sum(effect.money) FROM etf.ledger_effects effect WHERE effect.portfolio_id=lot.portfolio_id AND effect.lot_id=lot.lot_id AND pg_catalog.right(effect.effect_type,5)='Basis'),0) AS remaining_basis FROM etf.ledger_lots lot WHERE lot.portfolio_id=portfolio AND lot.instrument_id=payload ->> 'instrumentId' ORDER BY acquired_at,ledger_sequence,lot_id FOR SHARE LOOP
      EXIT WHEN remaining=0; IF lot_record.available<=0 THEN CONTINUE; END IF;
      IF least(remaining,lot_record.available)=lot_record.available THEN allocated:=lot_record.remaining_basis; ELSE allocation_value:=lot_record.original_basis*(least(remaining,lot_record.available)/lot_record.original_quantity); allocated:=trunc(allocation_value,8)+CASE WHEN allocation_value-trunc(allocation_value,8)>0.000000005 OR (allocation_value-trunc(allocation_value,8)=0.000000005 AND mod(trunc(allocation_value,8)*100000000,2)<>0) THEN 0.00000001 ELSE 0 END; END IF;
  ...
  INSERT INTO etf.ledger_effects (portfolio_id,transaction_id,effect_ordinal,effect_type,instrument_id,lot_id,source_effect_id,quantity,money,rate)
    SELECT portfolio,transaction_identity,effect.effect_ordinal,effect.effect_type,effect.instrument_id,effect.lot_id,effect.source_effect_id,effect.quantity,effect.money,effect.rate
     ...
     ORDER BY effect.effect_ordinal;
  IF kind='BuyFill' THEN
    INSERT INTO etf.ledger_lots VALUES (portfolio,(payload ->> 'fillId')::uuid,payload ->> 'instrumentId',(payload ->> 'simulatedAt')::timestamp with time zone,sequence_number,quantity_value,gross_value+fee_value);
  END IF;
...
BEGIN
  ...
  IF transition_name='OT-01' AND (jsonb_typeof(payload #> '{transitionPayload,quantity}') IS DISTINCT FROM 'string' OR payload #>> '{transitionPayload,quantity}' !~ '^(0|[1-9][0-9]*)\.[0-9]{10}$' OR jsonb_typeof(payload #> '{transitionPayload,unitPrice}') IS DISTINCT FROM 'string' OR payload #>> '{transitionPayload,unitPrice}' !~ '^(0|[1-9][0-9]*)\.[0-9]{10}$') THEN RAISE EXCEPTION 'APPLICATION_REQUEST_INVALID' USING ERRCODE='22023'; END IF;
  IF transition_name IN ('OT-05','OT-06','OT-09') AND (jsonb_typeof(payload #> '{transitionPayload,quantity}') IS DISTINCT FROM 'string' OR payload #>> '{transitionPayload,quantity}' !~ '^(0|[1-9][0-9]*)\.[0-9]{10}$' OR jsonb_typeof(payload #> '{transitionPayload,unitPrice}') IS DISTINCT FROM 'string' OR payload #>> '{transitionPayload,unitPrice}' !~ '^(0|[1-9][0-9]*)\.[0-9]{10}$' OR jsonb_typeof(payload #> '{transitionPayload,fee}') IS DISTINCT FROM 'string' OR payload #>> '{transitionPayload,fee}' !~ '^(0|[1-9][0-9]*)\.[0-9]{8}$') THEN RAISE EXCEPTION 'APPLICATION_REQUEST_INVALID' USING ERRCODE='22023'; END IF;
...
SET search_path = pg_catalog, etf
AS $function$
DECLARE verified boolean; audit_outcome text; audit_error text; audit_result jsonb; portfolio uuid; authoritative_cash numeric(28,8); authoritative_realized_pnl numeric(28,8); authoritative_total_equity numeric(28,8); authoritative_lots jsonb; cached_lots jsonb; authoritative_positions jsonb; cached_positions jsonb; authoritative_valued_positions jsonb; cached_valued_positions jsonb; authoritative_allocations jsonb; cached_allocations jsonb; authoritative_portfolio_version bigint; authoritative_precision_policy_version text; authoritative_baseline_version text;
BEGIN
  IF session_user<>'projection_runtime' OR payload IS NULL OR jsonb_typeof(payload)<>'object' OR NOT payload ?& ARRAY['allocations','asOf','baselineVersion','cash','keyIdentifier','lots','portfolioId','portfolioVersion','precisionPolicyVersion','positions','realizedPnL','reconciliationState','sourceCommitmentHash','totalEquity','valuationSnapshotId'] OR payload-ARRAY['allocations','asOf','baselineVersion','cash','keyIdentifier','lots','portfolioId','portfolioVersion','precisionPolicyVersion','positions','realizedPnL','reconciliationState','sourceCommitmentHash','totalEquity','valuationSnapshotId']::text[]<>'{}'::jsonb THEN RAISE EXCEPTION 'permission denied' USING ERRCODE='42501'; END IF;
  IF jsonb_typeof(payload -> 'allocations') IS DISTINCT FROM 'array' OR jsonb_typeof(payload -> 'lots') IS DISTINCT FROM 'array' OR jsonb_typeof(payload -> 'positions') IS DISTINCT FROM 'array' OR jsonb_typeof(payload -> 'cash') IS DISTINCT FROM 'string' OR payload ->> 'cash' !~ '^-?(0|[1-9][0-9]*)\.[0-9]{8}$' OR jsonb_typeof(payload -> 'realizedPnL') IS DISTINCT FROM 'string' OR payload ->> 'realizedPnL' !~ '^-?(0|[1-9][0-9]*)\.[0-9]{8}$' OR jsonb_typeof(payload -> 'totalEquity') IS DISTINCT FROM 'string' OR payload ->> 'totalEquity' !~ '^-?(0|[1-9][0-9]*)\.[0-9]{8}$' OR payload ->> 'portfolioVersion' !~ '^(0|[1-9][0-9]*)$' OR payload ->> 'asOf' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z$' THEN RAISE EXCEPTION 'APPLICATION_REQUEST_INVALID' USING ERRCODE='22023'; END IF;
  portfolio:=(payload ->> 'portfolioId')::uuid;
  PERFORM pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('etf:portfolio:' || portfolio::text,0));
  SELECT portfolio_version,precision_policy_version,baseline_version INTO authoritative_portfolio_version,authoritative_precision_policy_version,authoritative_baseline_version FROM etf.portfolios WHERE portfolio_id=portfolio;
  IF NOT FOUND THEN RAISE EXCEPTION 'LEDGER_RECONCILIATION_FAILED' USING ERRCODE='P0001'; END IF;
  ...
  SELECT COALESCE(sum(money) FILTER (WHERE pg_catalog.right(effect_type,4)='Cash'),0),COALESCE(sum(money) FILTER (WHERE pg_catalog.right(effect_type,11)='RealizedPnL'),0) INTO authoritative_cash,authoritative_realized_pnl FROM etf.ledger_effects WHERE portfolio_id=portfolio;
  SELECT COALESCE(jsonb_agg(jsonb_build_object('lotId',lot_id::text,'instrumentId',instrument_id,'quantity',to_char(open_quantity,'FM99999999999999990.0000000000'),'basis',to_char(open_basis,'FM99999999999999990.00000000')) ORDER BY lot_id),'[]'::jsonb) INTO authoritative_lots FROM (SELECT lot.lot_id,lot.instrument_id,sum(effect.quantity) FILTER (WHERE pg_catalog.right(effect.effect_type,8)='Position') AS open_quantity,sum(effect.money) FILTER (WHERE pg_catalog.right(effect.effect_type,5)='Basis') AS open_basis FROM etf.ledger_lots lot JOIN etf.ledger_effects effect ON effect.portfolio_id=lot.portfolio_id AND effect.lot_id=lot.lot_id WHERE lot.portfolio_id=(payload ->> 'portfolioId')::uuid GROUP BY lot.lot_id,lot.instrument_id HAVING sum(effect.quantity) FILTER (WHERE pg_catalog.right(effect.effect_type,8)='Position')<>0) open_lots;
  SELECT COALESCE(jsonb_agg(lot ORDER BY lot ->> 'lotId'),'[]'::jsonb) INTO cached_lots FROM jsonb_array_elements(payload -> 'lots') lot;
  SELECT COALESCE(jsonb_agg(jsonb_build_object('instrumentId',instrument_id,'quantity',to_char(quantity,'FM99999999999999990.0000000000'),'basis',to_char(basis,'FM99999999999999990.00000000')) ORDER BY instrument_id),'[]'::jsonb) INTO authoritative_positions FROM (SELECT instrument_id,sum(quantity) FILTER (WHERE pg_catalog.right(effect_type,8)='Position') AS quantity,sum(money) FILTER (WHERE pg_catalog.right(effect_type,5)='Basis') AS basis FROM etf.ledger_effects WHERE portfolio_id=(payload ->> 'portfolioId')::uuid AND instrument_id IS NOT NULL GROUP BY instrument_id HAVING sum(quantity) FILTER (WHERE pg_catalog.right(effect_type,8)='Position')<>0) positions;
  SELECT COALESCE(jsonb_agg(position - ARRAY['unitValue','valuation','unrealizedPnL']::text[] ORDER BY position ->> 'instrumentId'),'[]'::jsonb) INTO cached_positions FROM jsonb_array_elements(payload -> 'positions') position;
  SELECT COALESCE(jsonb_agg(jsonb_build_object('instrumentId',instrument_id,'quantity',to_char(quantity,'FM99999999999999990.0000000000'),'basis',to_char(basis,'FM99999999999999990.00000000'),'unitValue',unit_value,'valuation',to_char(valuation,'FM99999999999999990.00000000'),'unrealizedPnL',to_char(valuation-basis,'FM99999999999999990.00000000')) ORDER BY instrument_id),'[]'::jsonb) INTO authoritative_valued_positions FROM (SELECT ledger_position.*,cached_position.unit_value,trunc(raw_valuation,8)+CASE WHEN raw_valuation-trunc(raw_valuation,8)>0.000000005 OR (raw_valuation-trunc(raw_valuation,8)=0.000000005 AND mod(trunc(raw_valuation,8)*100000000,2)<>0) THEN 0.00000001 ELSE 0 END AS valuation FROM (SELECT instrument_id,sum(quantity) FILTER (WHERE pg_catalog.right(effect_type,8)='Position') AS quantity,sum(money) FILTER (WHERE pg_catalog.right(effect_type,5)='Basis') AS basis FROM etf.ledger_effects WHERE portfolio_id=(payload ->> 'portfolioId')::uuid AND instrument_id IS NOT NULL GROUP BY instrument_id HAVING sum(quantity) FILTER (WHERE pg_catalog.right(effect_type,8)='Position')<>0) ledger_position JOIN (SELECT position ->> 'instrumentId' AS instrument_id,position ->> 'unitValue' AS unit_value FROM jsonb_array_elements(payload -> 'positions') position) cached_position USING (instrument_id) CROSS JOIN LATERAL (SELECT ledger_position.quantity*cached_position.unit_value::numeric AS raw_valuation) valuation_input) valued_positions;
  SELECT COALESCE(jsonb_agg(position ORDER BY position ->> 'instrumentId'),'[]'::jsonb) INTO cached_valued_positions FROM jsonb_array_elements(payload -> 'positions') position;
  SELECT authoritative_cash+COALESCE(sum((position ->> 'valuation')::numeric),0) INTO authoritative_total_equity FROM jsonb_array_elements(authoritative_valued_positions) position;
  SELECT COALESCE(jsonb_agg(jsonb_build_object('sellTransactionId',sell_transaction_id::text,'effectOrdinal',effect_ordinal,'lotId',lot_id::text,'consumedQuantity',to_char(consumed_quantity,'FM99999999999999990.0000000000'),'allocatedBasis',to_char(allocated_basis,'FM99999999999999990.00000000')) ORDER BY sell_transaction_id,lot_id,effect_ordinal),'[]'::jsonb) INTO authoritative_allocations FROM etf.ledger_allocations WHERE portfolio_id=(payload ->> 'portfolioId')::uuid;
  SELECT COALESCE(jsonb_agg(allocation ORDER BY allocation ->> 'sellTransactionId',allocation ->> 'lotId',(allocation ->> 'effectOrdinal')::bigint),'[]'::jsonb) INTO cached_allocations FROM jsonb_array_elements(payload -> 'allocations') allocation;
  IF (payload ->> 'portfolioVersion')::bigint IS DISTINCT FROM authoritative_portfolio_version OR payload ->> 'precisionPolicyVersion' IS DISTINCT FROM authoritative_precision_policy_version OR payload ->> 'baselineVersion' IS DISTINCT FROM authoritative_baseline_version OR (payload ->> 'cash')::numeric IS DISTINCT FROM authoritative_cash OR (payload ->> 'realizedPnL')::numeric IS DISTINCT FROM authoritative_realized_pnl OR (payload ->> 'totalEquity')::numeric IS DISTINCT FROM authoritative_total_equity OR cached_lots IS DISTINCT FROM authoritative_lots OR cached_positions IS DISTINCT FROM authoritative_positions OR cached_valued_positions IS DISTINCT FROM authoritative_valued_positions OR cached_allocations IS DISTINCT FROM authoritative_allocations OR EXISTS (SELECT 1 FROM etf.portfolio_projections projection WHERE projection.portfolio_id=(payload ->> 'portfolioId')::uuid AND projection.as_of=(payload ->> 'asOf')::timestamp with time zone AND projection.valuation_snapshot_id<>(payload ->> 'valuationSnapshotId')::uuid) THEN RAISE EXCEPTION 'LEDGER_RECONCILIATION_FAILED' USING ERRCODE='P0001'; END IF;
...
EXCEPTION WHEN invalid_text_representation OR numeric_value_out_of_range OR datetime_field_overflow OR invalid_parameter_value THEN RAISE EXCEPTION 'APPLICATION_REQUEST_INVALID' USING ERRCODE='22023';
END;
$function$;
...
GRANT SELECT (portfolio_id, effect_type, instrument_id, lot_id, quantity, money) ON etf.ledger_effects TO projection_owner;
GRANT SELECT (portfolio_id, lot_id, instrument_id) ON etf.ledger_lots TO projection_owner;
GRANT SELECT (portfolio_id, sell_transaction_id, effect_ordinal, lot_id, consumed_quantity, allocated_basis) ON etf.ledger_allocations TO projection_owner;
GRANT SELECT (portfolio_id, portfolio_version, precision_policy_version, baseline_version) ON etf.portfolios TO projection_owner;
